...
export default function KebabMenu() {
  ...
  return (
    <div ref={ref} style={{ position: 'fixed', top: 'calc(env(safe-area-inset-top, 0px) + 10px)', right: 'max(12px, env(safe-area-inset-right, 0px))', zIndex: 11000 }}>
      <button aria-label="メニュー" onClick={() => setOpen(v => !v)} style={{ width: 40, height: 40, borderRadius: 12, background: 'rgba(255,255,255,0.9)', border: '1px solid #ddd', boxShadow: '0 4px 16px rgba(0,0,0,.08)', display: 'grid', placeItems: 'center', cursor: 'pointer', backdropFilter: 'saturate(120%) blur(6px)' }}>
      ...
      {open && (
        <div style={{ position: 'absolute', top: 48, right: 0, width: 260, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, boxShadow: '0 20px 40px rgba(0,0,0,.18)', overflow: 'hidden' }}>
          ...
          <MenuItem href="/pair" onClick={() => setOpen(false)}>ペア機能</MenuItem>
          ...
          }} style={itemStyle(true)}>ログアウト</button>
        </div>
      )}
    </div>
  );
...
function MenuItem({ href, children, onClick }: { href: string; children: React.ReactNode; onClick?: () => void }) {
  return <Link href={href} onClick={onClick} style={itemStyle()}>{children}</Link>;
}

function itemStyle(danger = false): React.CSSProperties {
  return { display: 'block', width: '100%', textAlign: 'left', padding: '12px 14px', fontWeight: 700, color: danger ? '#ef4444' : '#111827', textDecoration: 'none', background: '#fff' };
}
