  ...
  return <main className="stay-map-page">
    <section className="stay-map-content">
      ...
      <div className="stay-spot-list">{spots.map((spot, index) => <article className="stay-spot-card" key={spot.id} onClick={() => openSpot(spot)}>
        <button className="stay-card-main" aria-label={`${spot.name}の詳細を見る`}><div className="stay-card-image"><img src={spot.image_url || FALLBACK_IMAGE} alt={spot.name}/><span>{index + 1}</span>{spot.is_featured && <b>★ 宿主おすすめ</b>}</div><div className="stay-card-copy"><small>{spot.categories.map(item => item.name).join(" · ") || "おすすめ"}</small><h3>{spot.name}</h3><p><em>地元民からの一言</em>{spot.host_comment}</p><div>{travelLabel(stay, spot)}</div></div></button>
