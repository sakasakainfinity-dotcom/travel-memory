...
export default function LoginPage() {
  ...
  async function loginWithEmail(e: React.FormEvent) {
    ...
    try {
      ...
      alert("メールを送ったよ。受信箱のリンクを開いてサインインしてね。");
    } catch (err: any) {
      alert(err?.message ?? "メール送信に失敗したよ。");
    } finally {
      setBusy(false);
    }
  }
      ...
      <div style={styles.card}>
        <div style={styles.logoWrap}>
          ...
          <svg width="48" height="48" viewBox="0 0 48 48" aria-hidden>
            ...
            <circle cx="24" cy="24" r="22" fill="none" stroke="url(#g)" strokeWidth="2.5" />
            <path
        ...
        <p style={styles.subtitle}>ログインすると、地図と投稿機能が使えるようになるよ。</p>

        <button onClick={loginWithGoogle} disabled={busy} style={{ ...styles.btn, ...styles.btnPrimary }}>
        ...
        <form onSubmit={loginWithEmail} style={{ display: "grid", gap: 10 }}>
          <input
            type="email"
            placeholder="メールアドレス"
            value={email}
            ...
            style={styles.input}
          />
          <button type="submit" disabled={busy} style={{ ...styles.btn, ...styles.btnGhost }}>
            リンクをメールで受け取る
          </button>
        </form>

        <p style={styles.note}>メールはマジックリンク（または6桁コード）でサインインできるよ。</p>
      </div>
...
const styles: Record<string, React.CSSProperties> = {
  root: {
    ...
    minHeight: "100svh",
    overflow: "hidden",
    background: "linear-gradient(120deg, #0b1020 0%, #0e1733 60%, #0b1b3f 100%)",
    display: "grid",
    placeItems: "center",
    padding: 24,
  },
  ...
  subtitle: { color: "rgba(230,238,252,0.7)", margin: "0 0 16px", lineHeight: 1.6, fontSize: 14 },
  btn: {
    width: "100%",
    padding: "12px 14px",
    borderRadius: 12,
