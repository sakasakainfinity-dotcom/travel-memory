  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // 各 place の「最初の1枚」だけ取りたいので、created_at 昇順で並べた上で
        // まだサムネが未セットの place に限って最初の1件を採用
        const thumbnailBy: Record<string, string> = {};
        if (ids.length > 0) {
          ...
          if (!ePh && phs) {
            for (const ph of phs as { place_id: string; file_url: string }[]) {
              if (!thumbnailBy[ph.place_id]) {
                thumbnailBy[ph.place_id] = ph.file_url; // 最初に見つかった1枚だけセット
              }
            }
          } else {
            console.error(ePh);
        ...
        setItems(ps.map(p => ({
          ...
          lat: p.lat,
          lng: p.lng,
          thumbnail: thumbnailBy[p.id] ?? null,
        })));
      } catch (err) {
        console.error(err);
        setItems([]);
